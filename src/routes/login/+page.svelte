<script lang="ts">
	...
	async function handleSignIn() {
		...
		try {
			...
			// Same device check as signup, run on every login per doctrine
			// ("checked at login/auth time, not per-session" — meaning
			// every login, not just the first one).
			...
			if (!deviceResponse.ok) {
				error = deviceResult.message ?? 'Failed to verify device';
				loading = false;
				return;
			}
	...
	async function removeDeviceAndRetry(deviceId: number) {
		...
		try {
			...
			const deviceHash = await computeDeviceFingerprint();
			const deviceLabel = guessDeviceLabel();
			const retryResponse = await fetch('/api/check-device', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ device_hash: deviceHash, device_label: deviceLabel })
			});
			const retryResult = await retryResponse.json();
			if (retryResponse.ok && retryResult.allowed) {
				window.location.href = '/dashboard';
			} else {
				loading = false;
			}
